...
export const createExpense: ControllerFn = async (req, res, next) => {
  ...
  if (expenseName.includes("Salary") && !empId) {
    return next(
      new ErrorHandler("Salary Expense Must Be Provide An Employee", 400)
    );
  }
  ...
  if (employee && employee.empSalary < expenseCost) {
    return next(
      new ErrorHandler(
        "Salary Expense Must Be Less Than Employee Basic Salary",
        400
      )
    );
  }
...
export const getExpenses: ControllerFn = async (req, res, _next) => {
  if (req.showroomId) {
    ...
    res
      .status(200)
      .json(await Expenses.find({ relations: { showroom: true } }));
  }
};
